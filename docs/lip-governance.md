...
|---|---|---|---|---|---|---|
| `2026-05-31-01` | 2026-05-31 | B + E + D | **LIQ STRESS restoration (implementation completion / bug fix).** Re-enabled the existing `liq_stress` emit and added a dedicated always-on reader on `wss://fstream.binance.com/market/ws/!forceOrder@arr` feeding `state.liquidations` for tracked symbols (the main `/stream`+SUBSCRIBE socket is untouched). Root cause was endpoint **class** (`/market` delivers forceOrder; `/public`/default do not) — isolation-matrix verified; the earlier "external IP block" finding is falsified. No new metric/table/frontend; Phase 2/3A/3B emits unchanged. **Scope guard (load-bearing):** the same tape feeds the `liq_spike` resiliency trigger, so a flag `intelligence.LIQ_SPIKE_RESILIENCY_ENABLED = False` gates it OFF — restoring LIQ STRESS does **not** reactivate any CLOSED resiliency behaviour. liq_spike reactivation (flag→True) is a **separate, not-yet-authorized** governed change. | Implementation completion of a pre-existing primitive + latent endpoint-class fix. The Class D (runtime: new always-on connection) is constrained to LIQ STRESS by the guard; the resiliency-affecting Class D (liq_spike) is explicitly deferred. Reader excluded from run()'s FIRST_COMPLETED set (cannot disrupt ingestion). Operator-authorized; **not yet deployed** (implementation under review). | Nikita Oliinyk / pending review | `/market` endpoint is non-standard/undocumented — best-effort, may change; must degrade to honest absence. liq_stress determinism unchanged (60s rolling sum). |
