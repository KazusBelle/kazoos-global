...
- [`docs/lip-execution-validation.md`](lip-execution-validation.md) — execution validation contract for exec_impact: divergence measurement, replay edge cases, allowed-claims contract, maturity levels. Companion to §9.5 + [`lip-metric-registry.md`](lip-metric-registry.md) §A.5.
- [`docs/lip-governance.md`](lip-governance.md) — system governance & change-control contract: change classification, forbidden changes, replay stability, calibration lifecycle, observation→action firewall, composite contract, layer versioning, maturity gates, audit trail. Constrains how the architecture below is allowed to evolve.
- [`docs/lip-venue-quality.md`](lip-venue-quality.md) — **DESIGN CANDIDATE** (no runtime code). Per-(venue, symbol, window) reliability layer specification: 10 measurement dimensions, cross-venue status enum (reuses §14.8 TZ proposal), refusal conditions, blind-spot ceilings, UI language discipline. Class A under [`lip-governance.md`](lip-governance.md); implementation NOT AUTHORIZED during Observation Period.
- [`docs/lip-regime-engine.md`](lip-regime-engine.md) — Regime Engine / Market State Transition Layer hardening contract for the **already-implemented** `market_state_transitions` observational layer. Formalizes `coordinated_state` taxonomy, transition verdict precedence (REVERSED/FLICKER/ACCELERATING/PERSISTENT), confidence formula, refusal-first invariant, common-shock blind spot (NOT IMPLEMENTED), banned directional vocabulary. Companion to §B.7 in [`lip-metric-registry.md`](lip-metric-registry.md).
- [`docs/lip-causal-propagation.md`](lip-causal-propagation.md) — Propagation / Event-Chain Reconstruction canonical companion for the **already-implemented** `propagation_graph` / `causal_propagation` / `structural_dependencies` / `influence_hierarchy` / `narrative_causality` (legacy name) functions. Decomposes the stack into seven primitives (propagation edge · temporal adjacency · dependency graph · event chain reconstruction · conditional propagation candidate · common-shock suppression · replay-bounded sequence reconstruction). Reframes "narrative causality" as "Event Chain Reconstruction" at the documentation tier; code rename is a deferred Class B candidate. Companion to §B.4 / §B.5 / §B.6 in [`lip-metric-registry.md`](lip-metric-registry.md) and to §3 in [`lip-epistemic-boundaries.md`](lip-epistemic-boundaries.md).
- [`docs/lip-ontology-boundaries.md`](lip-ontology-boundaries.md) — Cross-cutting ontology hardening companion. Enumerates the ontology claims the platform is **not allowed to make** ("the market is", "true liquidity", "regime exists", "propagation transmitted", etc.) and the operational reformulations that are allowed instead. Carries the global invariant inherited by every per-layer companion that emits classifications: *"emits bounded observational classifications under current instrumentation constraints; does not establish authoritative market ontology"*.
- [`docs/lip-multi-operator.md`](lip-multi-operator.md) — Multi-operator workflow companion for the **already-implemented** investigation + replay + operator-event infrastructure (Layers 10 / 11 / 12). Formalizes attribution-without-authority, append-only audit lineage, replay-as-of-visibility discipline, operator disagreement legitimacy, contamination boundaries, forbidden consensus-truth / operator-ranking surfaces. Today's deployment is single-user; the contract binds as multi-operator usage activates.
- [`docs/lip-epistemic-branching.md`](lip-epistemic-branching.md) — Epistemic Branching & Contamination Governance companion. Class A governance over existing implicit branching surfaces (`origin_fingerprint`, `revision` on FROZEN snapshots, `reopen` lifecycle, supersession-via-active-flag). Formalizes fork legitimacy, parallel-unresolved legitimacy, stale-authority degradation (interpretive authority ≠ historical validity), supersession discipline (superseded ≠ false), replay-time multi-branch visibility, contamination governance (PRE/POST/BLINDED replay modes — last NOT IMPLEMENTED). Forbids canonical-truth / consensus-merge / branch-correctness scoring surfaces.
- [`docs/lip-semantic-vocabulary-boundaries.md`](lip-semantic-vocabulary-boundaries.md) — Cross-stack vocabulary-discipline companion. Codifies the 11-question test for semantic overclaim, modifier risk tiers (T1 operationally-pinned through T5 prestige/overclaim), per-stack replacement tables (causality / replay / confidence / quality / governance / "institutional"), and the rewrite playbook. Extends freeze §15 phrase-compression reference; per-layer banned-vocabulary tables remain authoritative for their domains.
- [`docs/lip-ingestion-contract.md`](lip-ingestion-contract.md) — Phase 1 ingestion audit companion (Class A, audit-only). Verifies the already-implemented ingestion layer (Binance WS depth20 + aggTrade, REST funding / mark / OI / exchange-info, Bybit v5 REST tickers, normalization, persistence, health tracking) against the Phase 1 contract: every event timestamped, append-only, explicit STALE / GAP / PARTIAL / UNKNOWN degradation, no silent interpolation, no synthetic reconstruction, no inferred liquidity. All criteria PASS. Honest limitations enumerated (`@forceOrder` upstream-disabled; WS-desync detection / timestamp-drift detector / calibration-version stamping all NOT IMPLEMENTED — Class B+E, NOT AUTHORIZED during Observation Period).
- [`docs/lip-liquidity-quality.md`](lip-liquidity-quality.md) — Liquidity-quality hardening companion for the already-implemented per-symbol stack (Credible Depth, exec_impact, Liquidation Stress, Resiliency / Recovery Time / Refill Velocity, Cross-Venue Divergence). Frames Credible Depth as **coarse survivorship heuristic** with explicit sub-400ms limitation; documents schema-level **tape isolation** that prevents liquidation flow from contaminating voluntary metrics (separate `state.trades` vs `state.liquidations` deques); confirms `crossex` is **read-only diagnostic — not wired into `fragility_score` or `crisis_genesis`** today (per freeze §14.8); enumerates UNKNOWN-propagation discipline; bans pressure/spring/signal/pump-setup/smart-money/manipulation vocabulary; approves survivorship/divergence/exhaustion-frequency/refill-velocity/cross-venue-disagreement. Class A.
- [`docs/lip-governance-consistency-audit.md`](lip-governance-consistency-audit.md) — Cross-surface compliance audit companion (Class A, audit-only). Grep-verified vocabulary discipline across API endpoint paths, UI strings, Python + TS code comments, DB schema enum values, alert text. Result: **6/6 surface categories PASS** with **8 residual drift incidents** classified — 3 Class A safe-fixable (comment lines in `liquidityIntelligence.ts`), 2 Class B deferred (`/research/signal-stats` + `/research/signal-reliability` endpoint renames), 3 Class B+E deferred (`AnnotationKind` enum values: `useful_signal` / `false_signal` / `manipulation`). Confirms no operator scoring, no venue trust scoring, no autonomous adaptation, no hidden predictive layers, no confidence theater. No urgent remediation required.
